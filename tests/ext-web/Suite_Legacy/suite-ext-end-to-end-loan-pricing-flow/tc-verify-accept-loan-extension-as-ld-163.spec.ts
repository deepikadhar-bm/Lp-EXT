...
test.describe('Suite: EXT: End To End Loan Pricing Flow', () => {
  ...
  test('TC: Verify Accept Loan Extension as LD', async ({ page }) => {
    ...
    await expect(page.locator("//APP-DLP-ELLIE-EXPORT-DIFFERENCES[1]/DIV[1]/DIV[2]/DIV[1]/TABLE[1]/TBODY[1]/TR[1]/TH[2]")).toContainText("Old Pricing");
    ...
    await expect(page.locator("//span[normalize-space()='Extension Requested']")).toBeVisible();
    await expect(page.locator("//button[contains(text(),\"Locked\")] | //button[text()=\"  Lock \"]")).toBeVisible();
    ...
    // [DISABLED] Verify that the element Locked Button is disabled
    // await expect(page.locator("//button[contains(text(),\"Locked\")] | //button[text()=\"  Lock \"]")).toBeVisible();
    ...
    await expect(page.locator("//li[contains(text(),\"Requested Days\")]")).toBeVisible();
