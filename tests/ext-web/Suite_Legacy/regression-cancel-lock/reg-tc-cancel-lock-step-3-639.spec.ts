...
test.describe('Regression: Cancel Lock', () => {
  ...
  test('REG TC: Cancel Lock Step 3', async ({ page }) => {
    ...
    await page.locator("//button[text()[normalize-space() = \"Relock ( Higher- Of)\"]] | //button[contains(text(), \"Relock \")]").click();
    await expect(page.locator("//button[text()[normalize-space() = \"Relock ( Higher- Of)\"]] | //button[contains(text(), \"Relock \")]")).toBeVisible();
    ...
    // [DISABLED] SG: Encompass for Reg
    // await stepGroups.stepGroup_SG_Encompass_for_Reg(page, vars);
    ...
    // [DISABLED] Convert the date from the Expiration Date from API in MM/dd/yyyy to MM/dd/yy and store it in a runtime Expire date from API
    // vars["Expire date from API"] = (() => {
    //   const d = new Date(String(vars["Expiration Date from API"]));
    ...
    // })();
    // [DISABLED] Verify if Lock Expiration Date From PPE EXT equals with Expire date from API
    // expect(String(vars["Lock Expiration Date From PPE EXT"])).toBe(String(vars["Expire date from API"]));
    // [DISABLED] Verify if Comments from API contains Comment
    // expect(String(vars["Comments from API"])).toBe(testData["Comment"]);
  });
});
