...
test.describe('EXT REG: Pricing a Loan (In Loan Pricer)', () => {
  ...
  test('REG TC: FHA Loans', async ({ page }) => {
    ...
    await page.locator("//li[contains(@class,\"active loan-types-fha\")]/span[text()=\" FHA \"]").evaluate(el => (el as HTMLElement).click());
    if (true) /* Element Active conventional is enabled */ {
      await page.locator("//li[@class=\"item active loan-type-conventional\"]").click();
    }
    await page.locator("(//label[contains(text(), \"Financed\")]/..//div)[last()]").waitFor({ state: 'visible' });
    await page.locator("//label[text()=\"UFMIP Fee Amount\" or contains(text(),\"UFMIP Amount\")]/..//input").waitFor({ state: 'visible' });
    ...
    if (true) /* Element SEARCH FIELDS Under Pricing is visible */ {
      vars["UFMIP Fee Amount"] = await page.locator("//label[text()=\"UFMIP Fee Amount\" or contains(text(),\"UFMIP Amount\")]/..//input").inputValue() || '';
    } else {
      vars["UFMIP Fee Amount"] = await page.locator("//label[text()=\"UFMIP Fee Amount\" or contains(text(),\"UFMIP Amount\")]/..//input").getAttribute('title') || '';
    }
    vars["Added Loan Amount"] = (parseFloat(String(vars["Loan Amount"])) + parseFloat(String(vars["UFMIP Fee Amount"]))).toFixed(0);
    vars[""] = String("Added Fee and Loan Amount").slice(0, 3) + "Added Fee and Loan Amount" + String("Added Fee and Loan Amount").slice(3);
    // [DISABLED] Add character .00 at position 7 in string Added Fee and Loan Amount and store into Added Fee and Loan Amount
    ...
    if (true) /* Element SEARCH FIELDS Under Pricing is not visible */ {
      await page.locator("(//label[contains(text(), \"Financed\")]/..//div)[last()]").scrollIntoViewIfNeeded();
      await page.locator("(//label[contains(text(), \"Financed\")]/..//div)[last()]").click();
      await page.locator("//option[normalize-space()=\"Not Financed\"]").click();
      ...
      await page.locator("//div[@role=\"document\"]//span[@aria-label=\"Close\"]").click();
    } else if (true) /* Element SEARCH FIELDS Under Pricing is visible */ {
