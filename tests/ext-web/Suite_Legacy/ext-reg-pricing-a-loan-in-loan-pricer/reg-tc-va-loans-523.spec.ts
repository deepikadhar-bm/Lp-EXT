...
test.describe('EXT REG: Pricing a Loan (In Loan Pricer)', () => {
  ...
  test('REG TC: VA Loans', async ({ page }) => {
    ...
    await page.locator("//li/span[text()=\" VA \"]").evaluate(el => (el as HTMLElement).click());
    if (true) /* Element Active conventional is enabled */ {
      await page.locator("//li[@class=\"item active loan-type-conventional\"]").click();
    }
    await page.locator("//select[@aria-label=\"Financed or Exempt?\"] | //select[@aria-label=\"Funding Fee Financed Type\"]").waitFor({ state: 'visible' });
    await page.locator("//label[contains(text(),\"Funding Fee Amount\")]/..//input").waitFor({ state: 'visible' });
    ...
    if (true) /* Element SEARCH FIELDS Under Pricing is visible */ {
      vars["Funding Fee Amount"] = await page.locator("//label[contains(text(),\"Funding Fee Amount\")]/..//input").inputValue() || '';
    } else {
      vars["Funding Fee Amount"] = await page.locator("//label[contains(text(),\"Funding Fee Amount\")]/..//input").getAttribute('title') || '';
    }
    vars["Added Loan Amount"] = (parseFloat(String(vars["Loan Amount"])) + parseFloat(String(vars["Funding Fee Amount"]))).toFixed(0);
    vars[""] = String("Added Fee and Loan Amount").slice(0, 3) + "Added Fee and Loan Amount" + String("Added Fee and Loan Amount").slice(3);
    vars[""] = String("Added Fee and Loan Amount").slice(0, 7) + "Added Fee and Loan Amount" + String("Added Fee and Loan Amount").slice(7);
    ...
    if (true) /* Element Funding Fee Financed input field has attribute ng-re */ {
      ...
      await page.locator("//label[contains(text(),\"Funding Fee Financed\")]/../div/select").click();
      ...
      await page.locator("//td[@data-label=\"COLUMN_NAME_ENUM.COST_TO_BORROWER\"]/div").click();
      await expect(page.locator("(//th[text()=\"Description\"]/../../../tbody/tr/th)[1]")).toContainText(vars["Added Fee Amount"]);
      await page.locator("//div[@role=\"document\"]//span[@aria-label=\"Close\"]").click();
      await page.locator("//label[contains(text(),\"Funding Fee Financed\")]/../div/select").click();
      await page.locator("//label[contains(text(),\"Financed\")]/../div/select/option[text()=\" EXEMPT \" or @ng-reflect-ng-value=\"EXEMPT\"]").click();
      await expect(page.locator("//label[contains(text(),\"Total Loan Amount\")]/..//input")).toHaveAttribute('title', vars["LoanAmount"]);
      await expect(page.locator("//label[contains(text(),\"Funding Fee Amount\")]/..//input")).toHaveAttribute('title', "0");
      await page.locator("//td[@data-label=\"COLUMN_NAME_ENUM.COST_TO_BORROWER\"]/div").click();
      await expect(page.locator("(//th[text()=\"Description\"]/../../../tbody/tr/th)[1]")).toContainText(vars["Loan Amount"]);
      await page.locator("//div[@role=\"document\"]//span[@aria-label=\"Close\"]").click();
    } else if (true) /* Element Financed or Exempt input field has attribute aria-la */ {
      ...
      await page.locator("//label[contains(text(),\"Financed\")]/../div/select/option[text()=\" EXEMPT \" or @ng-reflect-ng-value=\"EXEMPT\"]").click();
      await expect(page.locator("//label[contains(text(),\"Total Loan Amount\")]/..//input")).toHaveValue(vars["Loan Amount"]);
      await expect(page.locator("//label[contains(text(),\"Funding Fee Amount\")]/..//input")).toHaveValue("0");
      await page.locator("//button[text()[normalize-space() = \"Search\"]]").click();
      await page.locator("//td[@data-label=\"COLUMN_NAME_ENUM.COST_TO_BORROWER\"]/div").click();
      await expect(page.locator("(//th[text()=\"Description\"]/../../../tbody/tr/th)[1]")).toContainText(vars["Loan Amount"]);
      await page.locator("//div[@role=\"document\"]//span[@aria-label=\"Close\"]").click();
