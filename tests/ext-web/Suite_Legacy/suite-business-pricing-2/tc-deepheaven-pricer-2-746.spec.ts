...
test.describe('Suite: Business Pricing #2', () => {
  ...
  test('TC: Deepheaven Pricer #2', async ({ page }) => {
    ...
    for (let dataIdx = -1; dataIdx <= -1; dataIdx++) {
      ...
      await stepGroups.stepGroup_SG_Enter_Data_In_Loan_Information_Fields(page, vars);
      await stepGroups.stepGroup_SG_Enter_Data_In_Property_Fields(page, vars);
      await stepGroups.stepGroup_SG_Enter_Data_In_Income_Expenses_AMI_Fields(page, vars);
      await stepGroups.stepGroup_SG_Enter_Data_In_Product_Characteristics(page, vars);
      ...
      if (true) /* Element Active Eligible Tab is enabled */ {
        ...
        await expect(page.locator("//div[contains(text(),\"Non Prime\") or contains(text(),\"Expanded Prime\") or contains(text(),\"DSCR\")]")).toBeVisible();
        vars["Actual Program name"] = await page.locator("//div[contains(text(),\"Non Prime\") or contains(text(),\"Expanded Prime\") or contains(text(),\"DSCR\")]").textContent() || '';
