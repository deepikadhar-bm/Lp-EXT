...
test.describe('Regression: Reprice', () => {
  ...
  test('REG TC: Reprice Step 1', async ({ page }) => {
    ...
    // [DISABLED] SG: Check Eligible Loan
    ...
    vars["First Note Rate"] = await page.locator("//div[text()=\"Note Rate\"]/../div[2]").textContent() || '';
    vars["First Note Rate before reprice"] = String(vars["First Note Rate"]).trim();
