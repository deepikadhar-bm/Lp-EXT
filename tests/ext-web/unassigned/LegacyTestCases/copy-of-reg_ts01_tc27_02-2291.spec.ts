...
test.describe('Unassigned', () => {
  ...
  test('Copy of (REG_TS01_TC27_02)', async ({ page }) => {
    // Prerequisite: Copy of (REG_TS01_TC27_01)
    ...
    if (true) /* Element Configuration Icon is visible */ {
      ...
      await page.locator("(//div[@class=\"outer-accordian\"]/label[contains(@for,\"search-fieldsQuickPricer\")])[last()]").waitFor({ state: 'visible' });
      await page.locator("(//div[@class=\"outer-accordian\"]/label[contains(@for,\"search-fieldsQuickPricer\")])[last()]").click();
      await page.locator("(//div[contains(@class, \"custom-checkbox\")]/label[contains(@for, \"ausQuickPricer\")])[last()]").scrollIntoViewIfNeeded();
      if (true) /* Checkbox AUS Checkbox(Automation donot touch) is checked */ {
        await expect(page.locator("(//div[contains(@class, \"custom-checkbox\")]/input[contains(@id, \"ausQuickPricer\")])[last()]")).toBeVisible();
      } else {
        await expect(page.locator("(//div[contains(@class, \"custom-checkbox\")]/input[contains(@id, \"ausQuickPricer\")])[last()]")).toBeVisible();
        await page.locator("(//div[contains(@class, \"custom-checkbox\")]/input[contains(@id, \"ausQuickPricer\")])[last()]").evaluate(el => (el as HTMLElement).click());
        await expect(page.locator("(//div[contains(@class, \"custom-checkbox\")]/input[contains(@id, \"ausQuickPricer\")])[last()]")).toBeVisible();
      }
      if (true) /* Checkbox Non Conforming Checkbox(Automation donot touch test */ {
        vars["NonConforming"] = "checked";
      } else {
        vars["NonConforming"] = "unchecked";
      }
      if (true) /* Checkbox Guaranteed Underwriting Service Checkbox(Automation */ {
        vars["GuaranteedUnderwritingService"] = "checked";
      } else {
        vars["GuaranteedUnderwritingService"] = "unchecked";
      }
      if (true) /* Checkbox Manual Underwriting Checkbox(Automation donot touch */ {
        vars["ManualUnderwriting"] = "checked";
      } else {
        vars["ManualUnderwriting"] = "unchecked";
      }
      if (true) /* Checkbox LP Checkbox(Automation donot touch) is checked */ {
        vars["LP"] = "checked";
      } else {
        vars["LP"] = "unchecked";
      }
      if (true) /* Checkbox DU Checkbox(Automation donot touch) is checked */ {
        vars["DU"] = "checked";
      } else {
        vars["DU"] = "unchecked";
      }
