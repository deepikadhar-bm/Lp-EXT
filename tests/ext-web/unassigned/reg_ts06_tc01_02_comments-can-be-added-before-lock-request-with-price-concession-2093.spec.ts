...
test.describe('Unassigned', () => {
  ...
  test('REG_TS06_TC01_02_Comments can be added before Lock Request with Price Concession ny LO(AL)', async ({ page }) => {
    ...
    await page.locator("//h6[text()[normalize-space() = \"Lock Information\"]]").waitFor({ state: 'visible' });
    ...
    vars["AddedPrice"] = await page.locator("(//th[contains(text(), \"Custom Concession: $|AddedReason|\")]/..//td)[last()]").textContent() || '';
    expect(String(vars["AddedPrice"])).toBe(vars["PriceValueInPopup"]);
