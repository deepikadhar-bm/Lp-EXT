...
test.describe('Unassigned', () => {
  ...
  test('REG_TS04_TC11_if Guarantee Fee Financed : Yes Guarantee Fee should be added to Total Loan Amount and verify cost to borrower if Guarantee Fee Financed : No Guarantee Fee should NOT be added to Total L', async ({ page }) => {
    ...
    if (String(vars["UFMIPFinancedSelectedText"]).includes(String("Financed"))) {
      vars["FundingFeeAmount(Financed)VA"] = (parseFloat(String(vars["LoanAmount"])) + parseFloat(String(vars["USDAGuaranteeFeeValue"]))).toFixed(0);
    ...
    await expect(page.locator("//label[contains(text(),\"Total Loan Amount\")]/..//input")).toBeVisible();
    vars["LoanAmountNonFinancedUsdc"] = await page.locator("//label/../div/input[@id=\"loanAmount\"] | //input[contains(@placeholder,'Loan Amount')]").inputValue() || '';
    ...
    await stepGroups.stepGroup_Storing_actual_and_expected_Loan_in_CostToBorrower_Popup(page, vars);
    await stepGroups.stepGroup_Verifying_actual_and_expected_Loan_in_CostToBorrower_Popup(page, vars);
    vars["TotalLoanAmountinPopupUSDANonFinanced"] = await page.locator("(//tr[@class=\"currency\"]//th)[1]").textContent() || '';
    expect(String(vars["TotalLoanAmountinPopupUSDANonFinanced"])).toBe(vars["LoanAmountNonFinancedUsdc"]);
