...
test.describe('Unassigned', () => {
  ...
  test('REG_TS14_TC06_02_Request and apply Floating Concession', async ({ page }) => {
    ...
    await page.locator("//div[text()=\" Date \"]/../span[text()=\"Test Sigma Loan\"]/../span[text()=\"Floating Concession Requested\"]").waitFor({ state: 'visible' });
    await expect(page.locator("//div[text()=\" Date \"]/../span[text()=\"Test Sigma Loan\"]/../span[text()=\"Floating Concession Requested\"]")).toBeVisible();
    ...
    await stepGroups.stepGroup_Login_To_EXT_Application(page, vars);
    await stepGroups.stepGroup_SG_LP_Search_in_Pipeline_For_Created_Loan(page, vars);
    await stepGroups.stepGroup_SG_LP_Verifying_and_Selecting_Loan_for_Lock_Requested_Locked(page, vars);
    await stepGroups.stepGroup_SG_LP_Verifying_and_Selecting_Loan_for_Locked_Locked_StatusA(page, vars);
    await stepGroups.stepGroup_Verifying_Loan_Scenario_Page_and_Scroll_to_Lock_Information(page, vars);
