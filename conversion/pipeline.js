 ...
 *
 * Usage:
 *   node pipeline.js ext-web
 *
 * Project: EXT Application (Web)
 */
const fs = require('fs');
...
function generateEnvConfig(dataLoader, outputDir) {
  ...
  const lines = ['# Auto-generated from Testsigma environment variables - EXT Application', ''];
...
// Main execution
const projectArg = process.argv[2] || 'ext-web';

(async () => {
  await runPipeline(projectArg);
})().catch(err => {
